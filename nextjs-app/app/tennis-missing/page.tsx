...
export default function TennisMissing() {
  ...
  useEffect(() => {
    ...
    if (!isAuthenticated) return;
        ...
        // Initial fetch
        getMissingItems("tennis").then((items) => setMissingItems(items || []));
    
        // Real-time subscription
        const channel = supabase
          .channel("tennis-missing")
          .on(
            "postgres_changes",
            { event: "INSERT", schema: "public", table: "missing_items", filter: "page_type=eq.tennis" },
            (payload) => {
          ...
          .on(
            "postgres_changes",
            { event: "UPDATE", schema: "public", table: "missing_items", filter: "page_type=eq.tennis" },
            (payload) => {
          ...
          .on(
            "postgres_changes",
            { event: "DELETE", schema: "public", table: "missing_items", filter: "page_type=eq.tennis" },
            (payload) => {
      ...
      const handleClearAll = async () => {
        if (confirm("Are you sure you want to clear the completed missing items? There is no coming back from this...")) {
          ...
          await clearMissingItems("tennis");
        }
      };
  ...
  return (
    <div className={`tackle-theme container p-5 ${isDarkMode ? "dark" : ""}`}>
      <h1 className="mb-5 text-3xl font-bold text-center text-black dark:text-white">
        Tennis Warehouse Missing Items
      </h1>
