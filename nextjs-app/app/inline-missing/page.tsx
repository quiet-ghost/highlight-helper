...
export default function RunningMissing() {
  ...
  useEffect(() => {
    ...
    // Initial fetch
    getMissingItems("inline").then((items) => setMissingItems(items || []));

    // Real-time subscription
    const channel = supabase
      .channel("inline-missing")
      .on(
        "postgres_changes",
        {
          event: "INSERT",
  ...
  const handleClearAll = async () => {
    if (
      ...
      await clearMissingItems("inline");
    }
  };
  ...
  return (
    <div className={`inline-theme container p-5 ${isDarkMode ? "dark" : ""}`}>
      <h1 className="mb-5 text-3xl font-bold text-center text-black dark:text-white">
        Inline Warehouse Missing Items
      </h1>
