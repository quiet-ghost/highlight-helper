...
import { CartItem, runningList as initialRunningList } from "../../lib/runningCarts";

export default function Running() {
  const [cartInput, setCartInput] = useState<string>("");
  const [isDarkMode, setIsDarkMode] = useState<boolean>(false);
  const [mounted, setMounted] = useState<boolean>(false);
  const [runningList, setRunningList] = useState<CartItem[]>(initialRunningList);
  ...
  const handleReset = () => {
    setCartInput("");
    setRunningList((prevList) => prevList.map((item) => ({ ...item, count: 0 })));
  };
  ...
  return (
    <>
      ...
      <div className={`running-theme container p-5 ${isDarkMode ? "dark" : ""}`}>
        ...
        />
        <h4 className="text-xl font-semibold text-green-800 dark:text-gray-200 mb-2">
          Running List
        </h4>
        <ul className="mb-6">
